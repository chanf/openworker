...
const KIND_TABS = (t: TFn): { key: string; label: string }[] => [
  { key: "all", label: t("inbox.tabAll") },
  { key: "approval", label: t("inbox.tabApprovals") },
  { key: "question", label: t("inbox.tabQuestions") },
];
  ...
  const sessionChip = (it: InboxItem) => {
    ...
    return (
      <button
        ...
        title={exists ? t("inbox.openSession", { label }) : t("inbox.sessionUnavailable")}
        disabled={!exists}
        onClick={() =>
          ...
          {tab === "configure" ? (
            ...
            <>
              <div className="text-[12px] text-faint -mt-1 mb-4" data-testid="inbox-routing">
                {routing ? (
                  <span>{t("inbox.alsoDelivered", { label: routingLabel || "" })} </span>
                ) : slackConnected ? (
                  <span>{t("inbox.deliveredHereOnly")} </span>
                ) : (
                  <span>{t("inbox.connectSlackHint")} </span>
                )}
                <button
              ...
              {visible.length === 0 ? (
                <div className="manage-empty">
                  {items.length === 0 ? t("inbox.emptyAll") : t("inbox.emptyFilter")}
                </div>
              ) : null}
