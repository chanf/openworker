...
const SET_TABS = (t: TFn): { key: SetTab; label: string; icon: "sliders" | "code" | "mic" | "sparkle" | "book" }[] => [
  { key: "appearance", label: t("settings.tabGeneral"), icon: "sliders" },
  { key: "models", label: t("settings.tabModels"), icon: "code" },
  { key: "skills", label: t("settings.tabSkills"), icon: "book" },
  { key: "voice", label: t("settings.tabVoice"), icon: "mic" },
  { key: "personas", label: t("settings.tabPersonas"), icon: "sparkle" },
];
...
export function SettingsView({
  ...
  const tabs = personas ? SET_TABS(t) : SET_TABS(t).filter((tab) => tab.key !== "personas");
    ...
    <main className="flex-1 min-w-0 flex bg-paper">
      <nav className="page-subnav w-[208px] shrink-0 border-r border-line bg-panel/40 px-3 py-4">
        <div className="px-2 text-[13.5px] font-semibold mb-3 flex items-center gap-2">
          <Icon name="gear" size={16} /> {t("settings.settingsTitle")}
        </div>
        {tabs.map((entry) => {
...
const voiceError = (error: unknown, t: TFn) =>
  error instanceof Error ? error.message : typeof error === "string" ? error : t("settings.voiceErrorAction");
...
function VoiceInputSection() {
  ...
  const remove = async () => {
    if (!window.confirm(t("settings.voiceConfirmDelete"))) return;
    setError(null);
    try {
      publish(await deleteDictationModel());
      setTestTranscript("");
      ...
      {!desktop ? (
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-green-200 bg-green-50/70 px-4 py-3 text-[12.5px] text-green-800">
            <span className="font-medium">{t("settings.voicePrivateBold")}</span> {t("settings.voicePrivateBody")}
          </div>
          ...
          <div className={CARD}>
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[12px] text-muted mt-1">{status?.device_summary || t("settings.voiceCheckingCompat")}</div>
            ...
            <div className="border-t border-line bg-paper/50 px-4 py-3 grid grid-cols-2 gap-3 text-[12px] text-muted">
              <div><span className="block text-ink font-medium">{t("settings.voiceReqMac")}</span>{t("settings.voiceReqMacDetail")}</div>
              <div><span className="block text-ink font-medium">{t("settings.voiceReqWindows")}</span>{t("settings.voiceReqWindowsDetail")}</div>
              <div><span className="block text-ink font-medium">{t("settings.voiceReqMemory")}</span>{t("settings.voiceReqMemoryDetail")}</div>
              <div><span className="block text-ink font-medium">{t("settings.voiceReqProcessor")}</span>{t("settings.voiceReqProcessorDetail")}</div>
            </div>
          </div>
          ...
          <div className={CARD}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{t("settings.voiceWhisperModel")}</div>
                <div className="text-[12px] text-muted mt-0.5">
                  {status?.model_verified ? t("settings.voiceInstalledVerified", { bytes: formatBytes(status.model_bytes) }) : t("settings.voiceLocalModel", { bytes: formatBytes(status?.model_bytes || 147_964_211) })}
                </div>
              </div>
              {status?.model_verified ? (
                <>
                  <span className="text-[11.5px] px-2 py-1 rounded-full bg-green-50 text-green-700">{t("settings.voiceVerified")}</span>
                  <button className={BTN_BORDERED} onClick={() => void repair()}>{t("settings.voiceRepair")}</button>
                  <button className="text-[12px] text-red-600 px-2 py-2" onClick={() => void remove()}>{t("settings.voiceDelete")}</button>
                </>
              ) : downloading ? (
                <button className={BTN_BORDERED} onClick={() => void cancelDownload()}>{t("settings.voiceCancel")}</button>
              ) : phase === "verifying" ? (
                <span className="text-[12px] text-muted">{t("settings.voiceVerifying")}</span>
              ) : (
                <button className={BTN_ACCENT} disabled={!status?.supported} onClick={() => void download()}>{t("settings.voiceDownloadModel")}</button>
              )}
            </div>
            {downloading && (
              <div className="border-t border-line px-4 py-3">
                ...
                <div className="mt-1.5 text-[11.5px] text-muted flex"><span>{t("settings.voiceProgressOf", { a: formatBytes(progress?.downloaded_bytes || 0), b: formatBytes(progressTotal) })}</span><span className="ml-auto">{progressPercent}%</span></div>
              </div>
            )}
          </div>
          ...
          <div className={CARD}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[12px] text-muted mt-0.5">
                  {ready ? t("settings.voiceMicReadyDesc") : t("settings.voiceMicRecordDesc")}
                </div>
              </div>
              {ready && <span className="text-[11.5px] px-2 py-1 rounded-full bg-green-50 text-green-700">{t("settings.voiceReady")}</span>}
              <button className={BTN_BORDERED} disabled={!status?.supported || !status?.model_verified || phase === "transcribing"} onClick={() => void toggleTest()}>
                {status?.recording ? t("settings.voiceStopCheck") : phase === "transcribing" ? t("settings.voiceTranscribing") : ready ? t("settings.voiceTestAgain") : t("settings.voiceTestMic")}
              </button>
            </div>
            {status?.recording && <div className="border-t border-line px-4 py-3 text-[12px] text-accent" role="status">{t("settings.voiceListening")}</div>}
    ...
    <section>
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[13.5px] font-medium">{t("settings.browseGallery")}</span>
          <span className="block text-[12px] text-muted">
            {t("settings.galleryDesc")}
          </span>
        </span>
        <span className="text-[12.5px] text-accent shrink-0">{t("settings.galleryOpen")}</span>
      </button>
      {galleryOpen && (
        <GalleryModal
...
function AppearanceSection() {
  ...
  return (
    <section>
      <PanelHead title={t("settings.tabGeneral")} sub={t("settings.generalSub")} />

      <div className={CARD + " p-4 mb-4"}>
        <div className={FIELD_LABEL}>{t("settings.theme")}</div>
        <div className="seg mt-2.5" role="radiogroup" aria-label={t("settings.themeAppearance")}>
        ...
        <div className={FIELD_HELP}>{t("settings.themeAutoHelp")}</div>
      </div>
      ...
      <div className={CARD + " p-4 mb-4"}>
        ...
        <div className={FIELD_HELP}>{t("settings.langHelp")}</div>
      </div>
...
function TrustedWorkspacesCard() {
  ...
  const revoke = async (path: string) => {
    if (!window.confirm(t("settings.revokeConfirm", { path }))) return;
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="trusted-workspaces-card">
      ...
      {workspaces === null ? (
        <div className="text-[12px] text-muted mt-3">{t("settings.loadingDots")}</div>
      ) : workspaces.length === 0 ? (
        <div className="text-[12px] text-muted mt-3">{t("settings.noTrusted")}</div>
      ) : (
                ...
                <div className="text-[11.5px] text-muted mt-0.5">
                  {workspace.requested_commands.length
                    ? (workspace.requested_commands.length === 1
                        ? t("settings.cmdAllowancesOne", { n: workspace.requested_commands.length })
                        : t("settings.cmdAllowancesMany", { n: workspace.requested_commands.length }))
                    : t("settings.noCmdAllowances")}
    ...
    <span className="inline-flex items-center gap-2.5">
      {state === "found" ? (
        <button className={BTN_BORDERED} onClick={install} data-testid="settings-update-install">
          {t("settings.updateInstall", { version })}
        </button>
      ) : (
        <button
...
function TokenSavingsCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="token-savings-card">
      ...
      <div className="seg mt-2" role="radiogroup" aria-label={t("settings.pdfFallbackAria")} data-testid="pdf-fallback">
        <button
          className={pdf.pdf_fallback === "text" ? "active" : ""}
          onClick={() => save({ pdf_fallback: "text" })}
        >
          {t("settings.pdfExtractText")}
        </button>
        <button
          className={pdf.pdf_fallback === "images" ? "active" : ""}
          onClick={() => save({ pdf_fallback: "images" })}
        >
          {t("settings.pdfPageImages")}
        </button>
      </div>
      <div className={FIELD_HELP}>
...
function CompactionCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="compaction-card">
      ...
      <div className={FIELD_HELP}>{t("settings.compactionHelp")}</div>
      ...
      <div className="mt-3 flex items-center gap-5 flex-wrap">
        <label className="flex items-center gap-2.5">
          ...
          />
          <span className="text-[12.5px] text-muted">{t("settings.compactAtCtx")}</span>
        </label>
        <label className="flex items-center gap-2.5">
          ...
          />
          <span className="text-[12.5px] text-muted">{t("settings.compactTokens")}</span>
        </label>
      </div>
      <div className={FIELD_HELP}>{t("settings.compactionCapHelp")}</div>
      ...
      <div className="mt-3 flex items-center gap-2.5">
        ...
        >
          <option value="">{t("settings.compactionModelDefault")}</option>
          {models.map((m) => (
      ...
      <div className={FIELD_HELP}>{t("settings.compactionModelHelp")}</div>
    </div>
  );
}
...
function ContextBarCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="context-bar-card">
      <div className={FIELD_LABEL}>{t("settings.contextBarTitle")}</div>
      <label className="flex items-start gap-3 py-2">
        <input
          type="checkbox"
          className="mt-0.5"
        ...
        />
        <span>
          <span className="block text-[13px] text-ink">{t("settings.contextBarShow")}</span>
          <span className="block text-[12px] text-muted">
            {t("settings.contextBarDesc")}
          </span>
        </span>
      </label>
    </div>
  ...
  return (
    <div className={CARD + " p-4 mb-4"}>
      ...
      <label className="flex items-center gap-3 mt-2.5">
        <span className="text-[13px] text-ink">{t("settings.sidebarConvs")}</span>
        <input
          type="number"
          min={1}
          max={50}
      ...
      <div className={FIELD_HELP}>{t("settings.sidebarHelp")}</div>
    </div>
  );
}
...
function FilesCard() {
  ...
  const saveScratch = async () => {
    ...
    if (res.ok) {
      ...
      setScratchMsg(res.error || t("settings.filesSaveError"));
    }
  };
    ...
    <div className={CARD + " p-4 mb-4"}>
      <div className={FIELD_LABEL}>{t("settings.files")}</div>
        <div className="flex items-center gap-2 mt-2.5">
          ...
          />
          {desktop && (
            <button className={BTN_BORDERED} onClick={browseScratch} title={t("settings.filesBrowseTitle")}>
      ...
      <div className={FIELD_HELP}>{t("settings.filesHelp")}</div>
