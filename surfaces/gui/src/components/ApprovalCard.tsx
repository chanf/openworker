...
import { useI18n, type TFn, type Translations } from "../i18n";
...
// Map tool name → translation key; unknown tools fall back to the raw tool name.
const VERB_KEYS: Record<string, keyof Translations["approval"]> = {
  write_file: "verbWriteFile",
  replace_in_file: "verbReplaceInFile",
  apply_patch: "verbApplyPatch",
  apply_unified_diff: "verbApplyPatch",
  run_shell: "verbRunShell",
  send_message: "verbSendMessage",
  send_file: "verbSendFile",
};

function toolVerb(t: TFn, tool: string): string {
  const key = VERB_KEYS[tool];
  return key ? t(`approval.${key}`) : tool;
}
...
export function approvalActionLabels(t: TFn, name?: string): { allow: string; deny: string } {
  return name === "save_skill"
    ? { allow: t("approval.saveSkillAdd"), deny: t("approval.saveSkillNotNow") }
...
export function SaveSkillPreview({ args }: { args: any }) {
  ...
  return (
    <>
      ...
      <div className="approval-with">{t("approval.saveSkillFooter")}</div>
    </>
  );
}
...
export function scopeNote(
  ...
  if (name === "save_skill") return { text: t("approval.saveSkillScope"), external: false };
  if (category === "connector") return { text: t("approval.actsConnector"), external: true };
  if (EXTERNAL.has(name)) {
    ...
    return {
      text: t("approval.leavesMac", {
        dest: names[platform] || platform || t("approval.connectedChat"),
      }),
      external: true,
    };
  }
  ...
  return {
    text: t("approval.staysMac") + (overwrite ? t("approval.overwritesSuffix") : ""),
    external: false,
  };
}
        ...
        <button className="approval-prev-more" onClick={() => setAll((v) => !v)}>
          {all
            ...
            : lines.length > PREVIEW_LINES
              ? t("approval.showAllLines", { n: lines.length })
              : t("approval.showFull")}
        </button>
      )}
    </div>
  ...
  return (
    <div className="approval-btns">
      ...
      {offerStanding && (
        ...
        >
          {t("approval.alwaysAllowEveryTime")}
        </button>
      )}
      ...
      {!connector && !offerStanding && item.name !== "run_shell" && item.name !== "save_skill" && (
        <button
          className="btn"
          title={t("approval.alwaysToolTitle", { verb: toolVerb(t, item.name).toLowerCase() })}
...
export function ApprovalCard({
  ...
  return (
    <div className={"approval" + (scope.external ? " approval-external" : "") + dock}>
      ...
      {item.name === "send_file" && (
        <>
          <span className="approval-filechip">
            ...
            {String(item.args?.path ?? "").split("/").pop() || t("approval.fileFallback")}
            {item.args?.as_screenshot ? t("approval.asScreenshot") : ""}
          </span>
          {item.args?.comment && (
          ...
          {grants.map((g, i) => (
            <div className="approval-grant" key={i} data-access={g.access}>
              ...
              <span className="grant-line">
                {toolVerb(t, g.tool)} <code className="approval-tool">{g.target}</code>
                <span className="grant-note">
                  {g.access === "write" ? t("approval.grantWriteNote") : t("approval.grantReadNote")}
                </span>
              </span>
            </div>
          ))}
      ...
      {item.resolved ? (
        <div className="resolved">{t("approval.approved", { state: item.resolved.replace("_", " ") })}</div>
      ) : (
        <Buttons
          item={item}
          onApprove={onApprove}
